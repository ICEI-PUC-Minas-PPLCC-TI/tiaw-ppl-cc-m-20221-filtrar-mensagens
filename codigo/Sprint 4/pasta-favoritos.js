var pastas = [
    {
        'nome-input': 'Mensagens Favoritas', 'comment-input': 'Favoritos', 'corSelecionada': 'azul', 'cor': converteCor('azul'), 'mensagens':
            [{ 'foto': 'img1.jpg', 'nome': 'Pai', 'horario': '14:20', 'mensagem': 'Meu cpf: 123.456.789-00' },
            { 'foto': 'img2.jpg', 'nome': 'Cauã', 'horario': '14/06/22', 'mensagem': 'Vc vai no rolê dia 17?' },
            { 'foto': 'img3.jpg', 'nome': 'Léo', 'horario': '14/06/22', 'mensagem': 'Vamos no chalezinho hoje?' },
            { 'foto': 'img4.jpg', 'nome': 'André Campos', 'horario': '14/06/22', 'mensagem': 'Endereço: Rua Sergipe, 387, apto 901' },
            { 'foto': 'img5.jpg', 'nome': 'Tio Júlio', 'horario': '02/06/22', 'mensagem': 'Parabéns pela sua conquista, tenho muito orgulho de você!!' },
            { 'foto': 'img8.jpg', 'nome': 'Pedro Gonçalves', 'horario': '14/06/22', 'mensagem': 'Se cuida amor!' },
            { 'foto': 'img7.jpg', 'nome': 'Valéria Silva', 'horario': '14/06/22', 'mensagem': 'replit.com/@bruna-horta' },]
    }];
var pastaAtual = 0;
...
function atualizaRightSide(item) {
    ...
    if (pastas[item]['corSelecionada'] == 'plaranja' || pastas[item]['corSelecionada'] == 'plime' || pastas[item]['corSelecionada'] == 'pverde' || pastas[item]['corSelecionada'] == 'pazul' || pastas[item]['corSelecionada'] == 'proxo') {
        document.querySelector('#pasta-img').innerHTML = `<ion-icon name="star" class="pasta-black" style="background-color:${converteCor(pastas[item]['corSelecionada'])}";></ion-icon>`
    } else {
        document.querySelector('#pasta-img').innerHTML = `<ion-icon name="star" class="pasta-white" style="background-color:${converteCor(pastas[item]['corSelecionada'])}";></ion-icon>`
    }
    ...
    for (let i = 0; i < pastas.length; i++) {
        ...
        document.getElementById('pastas').innerHTML += `
            <div onclick="atualizaRightSide(${i})" class="block" id="pasta-item${i}">
                <div class="imgbox">
                <ion-icon name="star" class="${corPasta}" style="background-color:${converteCor(pastas[i]['corSelecionada'])}";></ion-icon>
                </div>
                <div class="details">
                    <div class="listHead">
