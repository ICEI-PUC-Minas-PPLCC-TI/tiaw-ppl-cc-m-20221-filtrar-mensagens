var pastas = [
    {
        'nome-input': 'Mensagens fofas', 'comment-input': '♥♥♥♥', 'corSelecionada': 'proxo', 'cor': converteCor('proxo'), 'mensagens':
            [{ 'foto': 'img5.jpg', 'nome': 'Tio Túlio', 'horario': '18:22', 'mensagem': 'Você é a melhor pessoa. Ponto mesmo, é isso a frase.' },
            { 'foto': 'img7.jpg', 'nome': 'Valéria Silva', 'horario': '14/06/22', 'mensagem': 'Te amo!♥' },
            { 'foto': 'img9.jpg', 'nome': 'Érica', 'horario': '14/06/22', 'mensagem': 'Cupon ifood: rtDf4w1 pq vc vale um salgado ♥' },
            { 'foto': 'img8.jpg', 'nome': 'Pedro Gonçalves', 'horario': '14/06/22', 'mensagem': 'Você é a melhor amiga da vida' },
            { 'foto': 'img1.jpg', 'nome': 'Pai', 'horario': '02/06/22', 'mensagem': 'Saudades de você!' }]
    },
    {
        'nome-input': 'Faltas', 'comment-input': 'Pare de vagabundar', 'corSelecionada': 'plaranja', 'cor': converteCor('plaranja'), 'mensagens':
            [{ 'foto': 'user.jpg', 'nome': 'Bruna Neves', 'horario': '16:40', 'mensagem': 'Meu Deus, eu to com 20% em AEDs, to ferrada!' },
            { 'foto': 'user.jpg', 'nome': 'Bruna Neves', 'horario': '11/06/22', 'mensagem': 'faltei LIP 11/06' },
            { 'foto': 'user.jpg', 'nome': 'Bruna Neves', 'horario': '06/06/22', 'mensagem': 'faltei cálculo 06/06' }]
    },
    {
        'nome-input': 'Aniversários', 'comment-input': 'Lembrar do presente', 'corSelecionada': 'pazul', 'cor': converteCor('pazul'), 'mensagens':
            [{ 'foto': 'img2.jpg', 'nome': 'Cauã', 'horario': '16:40', 'mensagem': 'Bernardo PUC: 23/09' },
            { 'foto': 'img9.jpg', 'nome': 'Érica', 'horario': '11/06/22', 'mensagem': 'Mateus: 15/12' },
            { 'foto': 'img4.jpg', 'nome': 'André Campos', 'horario': '06/06/22', 'mensagem': 'Bianca PUC: 13/02' },
            { 'foto': 'user.jpg', 'nome': 'Bruna Neves', 'horario': '11/06/22', 'mensagem': 'Beatriz: 09/10' }]
    }];
var pastaAtual = 0;
...
function atualizaRightSide(item) {
    ...
    if (pastas[item]['corSelecionada'] == 'plaranja' || pastas[item]['corSelecionada'] == 'plime' || pastas[item]['corSelecionada'] == 'pverde' || pastas[item]['corSelecionada'] == 'pazul' || pastas[item]['corSelecionada'] == 'proxo') {
        document.querySelector('#pasta-img').innerHTML = `<ion-icon name="pricetags-outline" class="pasta-black" style="background-color:${converteCor(pastas[item]['corSelecionada'])}";></ion-icon>`
    } else {
        document.querySelector('#pasta-img').innerHTML = `<ion-icon name="pricetags-outline" class="pasta-white" style="background-color:${converteCor(pastas[item]['corSelecionada'])}";></ion-icon>`
    }
    ...
    for (let i = 0; i < pastas.length; i++) {
        ...
        document.getElementById('pastas').innerHTML += `
            <div onclick="atualizaRightSide(${i})" class="block" id="pasta-item${i}">
                <div class="imgbox">
                <ion-icon name="pricetags-outline" class="${corPasta}" style="background-color:${converteCor(pastas[i]['corSelecionada'])}";></ion-icon>
                </div>
                <div class="details">
                    <div class="listHead">
...
document.getElementById('trash').addEventListener('click', (evento) => {
    let confirma = confirm(`Deseja deletar esta tag?`);
    if (confirma == true) {
